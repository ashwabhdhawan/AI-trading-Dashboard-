...
signal_filter = st.sidebar.multiselect(
    "Stock Signal", sorted(df["Stock_Signal"].unique()),
    default=sorted(df["Stock_Signal"].unique())
)
opt_filter = st.sidebar.multiselect(
    "Options Suggestion", sorted(df["Options_Suggestion"].unique()),
    default=sorted(df["Options_Suggestion"].unique())
)
tilt_filter = st.sidebar.multiselect(
    "BL Tilt", sorted(df["BL_Tilt"].dropna().unique()),
    default=sorted(df["BL_Tilt"].dropna().unique())
)
sort_by = st.sidebar.selectbox(
    "Sort by", ["BL_Posterior_annual","BL_Rank","Signal_Strength","RSI14","Vol20_ann"], index=0
)
